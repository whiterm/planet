...
FROM debian:${DEBIAN_VERSION} AS iptables-builder
ARG IPTABLES_VER
RUN set -ex && \
    apt update && \
	apt install -y git autoconf automake libtool libmnl-dev make pkg-config && \
        ...
        ./autogen.sh && \
        ./configure --disable-nftables --prefix=/tmp/iptables.local && \
        make && \
        make install
...
COPY --from=iptables-builder /tmp/iptables.local/ /usr/local/
...
RUN --mount=target=/root/.cache,type=cache --mount=target=/go/pkg/mod,type=cache \
	set -ex && \
	git clone https://github.com/docker/distribution -b ${DISTRIBUTION_VER} --depth 1 . && \
	GO111MODULE=off go build -a -installsuffix cgo -o /registry -ldflags "-X `go list ./version`.Version=${DISTRIBUTION_VER} -w" ./cmd/registry
...
COPY ./build.assets/makefiles/master/k8s-master/rootfs/etc/kubernetes/ /etc/kubernetes/
